/*
 * HealingMart Converter Self Test v1.17.0
 * Target release: HealingMart Converter v3.44.0
 *
 * Purpose:
 * - Batch-check registry structure
 * - Verify 670 active / 2 server-required and v3.44 stability markers
...
(function (w, d) {
  ...
  function testBrowserFeatures() {
    ...
    info(section, "안정화 검사", "검색 별칭·다중 토큰·파일 MIME 추천·Blogger 버전 동기화 마커 포함");
  ...
  async function run() {
    ...
    try {
      ...
      info(
        "테스트 도구",
        "검사 범위 안내",
        "정적/통합 검사를 자동화한 테스트입니다. 610개 변환기에 실제 파일을 넣는 전수 E2E 변환 테스트는 별도입니다."
      );
    } catch (error) {
