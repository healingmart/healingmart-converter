/*
 * HealingMart Converter Self Test v1.19.0
 * Target release: HealingMart Converter v3.46.0
 *
 * Purpose:
 * - Batch-check registry structure
 * - Verify 670 active / 2 server-required and v3.46 async engine stability markers
...
(function (w, d) {
  ...
  function testBrowserFeatures() {
    ...
    info(section, "안정화 검사", "비동기 엔진 로딩·렌더 경합·재시도·파일 선택 접근성 안정화 마커 포함");
  ...
  function buildTextReport() {
    ...
    lines.push(
      ...
      "현재 active 670개 변환기에 실제 사용자 파일을 넣어 모두 변환하는 전수 E2E 테스트를 대체하지는 않습니다."
    );
  ...
  async function run() {
    ...
    try {
      ...
      info(
        "테스트 도구",
        "검사 범위 안내",
        "정적/통합 검사를 자동화한 테스트입니다. 670개 변환기에 실제 파일을 넣는 전수 E2E 변환 테스트는 별도입니다."
      );
    } catch (error) {
