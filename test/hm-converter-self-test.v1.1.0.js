/*
 * HealingMart Converter Self Test v1.1.0
 * Target release: HealingMart Converter v3.28.0
 *
 * Purpose:
 * - Batch-check registry structure
 * - Verify 220 active / 2 server-required
...
(function (w, d) {
  ...
  async function testRegistry() {
    ...
    activeRouteFailures.length
      ...
      : pass(section, "active 라우팅 누락", "220개 모두 detail() 대상 확인");
  ...
  async function run() {
    ...
    try {
      ...
      info(
        "테스트 도구",
        "검사 범위 안내",
        "정적/통합 검사를 자동화한 테스트입니다. 220개 변환기에 실제 파일을 넣는 전수 E2E 변환 테스트는 별도입니다."
      );
    } catch (error) {
