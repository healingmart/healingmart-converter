/*
 * HealingMart Converter Image Engine v1.3.0
...
(function (w) {
  ...
  var avifEncoderPromise = null;

  async function ensureAvifEncoder() {
    if (!avifEncoderPromise) {
      avifEncoderPromise = import("https://esm.sh/@jsquash/avif@2.1.1").then(function (mod) {
        if (!mod || typeof mod.encode !== "function") {
          throw new Error("AVIF WebAssembly 인코더를 불러오지 못했습니다.");
        }
        return mod.encode;
  ...
  async function canvasToAvifBlob(canvas) {
    if (canvas.width * canvas.height > 25000000) {
      throw new Error("AVIF 변환은 2,500만 픽셀 이하 이미지에서 사용할 수 있습니다.");
    }

    var g = canvas.getContext("2d");
    if (!g) throw new Error("AVIF 변환용 Canvas를 사용할 수 없습니다.");

    var encode = await ensureAvifEncoder();
    var imageData = g.getImageData(0, 0, canvas.width, canvas.height);
    var output = await encode(imageData);

    if (!output || !output.byteLength) {
      throw new Error("AVIF 파일을 만들지 못했습니다.");
    }
  ...
  async function convertOne(file, x, options, ctx) {
    ...
    try {
      ...
      } else if (x.toFormat === "AVIF") {
        blob = await canvasToAvifBlob(canvas);
      } else {
  ...
  function formatNote(x) {
    ...
    if (to === "AVIF") {
      return "AVIF 출력은 @jsquash/avif 2.1.1의 libavif WebAssembly 인코더를 사용합니다. JPG/PNG를 서버로 업로드하지 않고 브라우저에서 처리하며 고해상도 이미지는 변환 시간이 길 수 있습니다.";
    }
    if (to === "GIF") {
      ...
      ctx.stage.innerHTML =
        '<div class="hm-fx-detail">' +
          ...
          '<div class="hm-eng-box">' +
            ...
            '<div class="hm-eng-version">Image Engine v1.3.0</div>' +
          '</div>' +
        '</div>';
