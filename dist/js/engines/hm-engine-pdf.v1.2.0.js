...
(function (w) {
  ...
  function docxParagraphXml(text, pageBreakAfter) {
    var lines = String(text || "").split("\n");
    var runs = lines.map(function (line, index) {
      var part = '<w:r><w:t xml:space="preserve">' + xmlEscapeDoc(line) + '</w:t></w:r>';
      if (index < lines.length - 1) part += '<w:r><w:br/></w:r>';
      return part;
    }).join("");

    if (pageBreakAfter) runs += '<w:r><w:br w:type="page"/></w:r>';
    return '<w:p>' + runs + '</w:p>';
  }

  async function createPdfTextDocx(extracted, ctx) {
    var JSZip = await ensureZip(ctx);
    var zip = new JSZip();

    var body = extracted.pages.map(function (page, index) {
      var header = docxParagraphXml(page.page + "페이지", false);
      var content = String(page.text || "").split("\n").map(function (line) {
        return docxParagraphXml(line, false);
      }).join("");
      var breakXml = index < extracted.pages.length - 1 ? '<w:p><w:r><w:br w:type="page"/></w:r></w:p>' : '';
      return header + content + breakXml;
    }).join("");
  ...
  async function openPdfDocx(x, ctx) {
    injectTextPdfStyle();
    ...
    ctx.stage.innerHTML =
      '<div class="hm-fx-detail">' +
        ...
        ctx.titleBlock(x) +
        '<div class="hm-pdf-box">' +
          '<div class="hm-pdf-drop" data-pd-drop><strong>PDF 파일을 선택하거나 끌어다 놓으세요</strong>' +
            '<span>선택 가능한 텍스트를 읽어 기본 DOCX 문서로 만듭니다.</span>' +
            '<input type="file" hidden data-pd-file accept=".pdf,application/pdf"></div>' +
          '<div class="hm-pdf-fileinfo" data-pd-info></div>' +
          '<div class="hm-pdf-options">' +
            '<div class="hm-pdf-field"><label>페이지 범위</label><input type="text" placeholder="예: 1-10,15 · 비우면 전체" data-pd-range></div>' +
            '<div class="hm-pdf-field"><label>최대 처리</label><select disabled><option>최대 300페이지</option></select></div>' +
          '</div>' +
          '<div class="hm-pdf-actions"><button class="hm-pdf-btn primary" type="button" data-pd-run disabled>DOCX 만들기</button></div>' +
          '<div class="hm-pdf-status" data-pd-status>스캔 이미지로만 구성된 PDF는 OCR 기능이 필요하므로 본문이 추출되지 않을 수 있습니다.</div>' +
          '<div class="hm-pdf-help">PDF의 텍스트를 페이지 순서대로 Word 문단으로 옮기는 기능입니다. 원본 PDF의 글꼴·도형·정밀한 레이아웃을 Word에서 그대로 복원하는 기능은 아닙니다.</div>' +
          '<div class="hm-pdf-version">PDF Engine v1.2.0</div>' +
        '</div></div>';

    var drop = ctx.stage.querySelector("[data-pd-drop]");
    var input = ctx.stage.querySelector("[data-pd-file]");
    var info = ctx.stage.querySelector("[data-pd-info]");
    var range = ctx.stage.querySelector("[data-pd-range]");
    var run = ctx.stage.querySelector("[data-pd-run]");
    var status = ctx.stage.querySelector("[data-pd-status]");

    async function choose(file) {
      try {
        var pages = await inspectPdfFile(file, ctx);
        selectedFile = file;
        info.style.display = "block";
        info.textContent = file.name + " · " + niceBytes(file.size) + " · " + pages + "페이지";
        run.disabled = false;
        status.textContent = "준비되었습니다.";
      } catch (error) {
        selectedFile = null;
    ...
    run.onclick = async function () {
      ...
      try {
        ...
        var chars = extracted.pages.reduce(function (sum, page) {
          return sum + String(page.text || "").replace(/\s/g, "").length;
        }, 0);

        if (chars < 10) throw new Error("추출된 텍스트가 거의 없습니다. 스캔 PDF라면 OCR이 필요합니다.");

        status.textContent = "DOCX 문서를 만드는 중입니다...";
        var blob = await createPdfTextDocx(extracted, ctx);
        downloadBlob(blob, safeBaseName(selectedFile.name) + ".docx");
        status.textContent = extracted.pages.length + "페이지를 DOCX로 변환했습니다.";
      } catch (error) {
        status.textContent = "변환 중 오류가 발생했습니다: " + (error.message || error);
      } finally {
        run.disabled = false;
  ...
  async function renderHtmlToPdfBlob(html, ctx, onProgress) {
    var JsPDF = await ensureJsPdf(ctx);
    var h2c = await ensureHtml2Canvas(ctx);
    ...
    host.innerHTML =
      ...
      'blockquote{margin:16px 0;padding:12px 16px;border-left:4px solid #8ca9e8;background:#f7f9fc}' +
      'pre,code{white-space:pre-wrap;word-break:break-word;font-family:Consolas,monospace}' +
      '</style>' + sanitizeBasicHtml(html);

    d.body.appendChild(host);

    try {
      if (host.scrollHeight > 16000) {
        throw new Error("문서가 너무 깁니다. 내용을 나누어 변환해 주세요.");
      }

      if (onProgress) onProgress("문서를 렌더링하는 중입니다...");
      ...
      var pagePixelHeight = Math.floor(canvas.width * 297 / 210);
      var totalPages = Math.ceil(canvas.height / pagePixelHeight);
      if (totalPages > 120) throw new Error("결과 PDF가 120페이지를 넘습니다.");

      var pdf = new JsPDF({ orientation:"portrait", unit:"mm", format:"a4", compress:true });

      for (var i = 0; i < totalPages; i += 1) {
        if (onProgress) onProgress((i + 1) + " / " + totalPages + " PDF 페이지 만드는 중...");
        if (i > 0) pdf.addPage("a4", "portrait");
  ...
  async function openDocxPdf(x, ctx) {
    injectTextPdfStyle();
    var selectedFile = null;
    ...
    ctx.stage.innerHTML =
      '<div class="hm-fx-detail">' +
        ...
        ctx.titleBlock(x) +
        '<div class="hm-pdf-box">' +
          '<div class="hm-pdf-drop" data-dp-drop><strong>DOCX 파일을 선택하거나 끌어다 놓으세요</strong>' +
            '<span>문서의 기본 내용 구조를 읽어 A4 PDF로 만듭니다.</span>' +
            '<input type="file" hidden data-dp-file accept=".docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document"></div>' +
          '<div class="hm-pdf-fileinfo" data-dp-info></div>' +
          '<div class="hm-pdf-actions"><button class="hm-pdf-btn primary" type="button" data-dp-run disabled>PDF 만들기</button></div>' +
          '<div class="hm-pdf-status" data-dp-status>DOCX의 제목·문단·목록·표 중심으로 변환합니다.</div>' +
          '<div class="hm-pdf-help">Word의 원본 페이지 렌더링을 복제하는 기능이 아닙니다. 머리글·바닥글·도형·복잡한 페이지 배치·일부 이미지와 스타일은 단순화되거나 제외될 수 있습니다.</div>' +
          '<div class="hm-pdf-version">PDF Engine v1.2.0</div>' +
        '</div></div>';

    var drop = ctx.stage.querySelector("[data-dp-drop]");
    var input = ctx.stage.querySelector("[data-dp-file]");
    var info = ctx.stage.querySelector("[data-dp-info]");
    var run = ctx.stage.querySelector("[data-dp-run]");
    var status = ctx.stage.querySelector("[data-dp-status]");

    function choose(file) {
      if (!file || !/\.docx$/i.test(file.name)) {
        selectedFile = null;
        run.disabled = true;
        status.textContent = "DOCX 파일을 선택해 주세요.";
        return;
      }
      if (file.size > 30 * 1024 * 1024) {
        selectedFile = null;
        run.disabled = true;
        status.textContent = "DOCX 파일은 30MB 이하만 처리할 수 있습니다.";
        return;
      }
      selectedFile = file;
      info.style.display = "block";
    ...
    run.onclick = async function () {
      ...
      run.disabled = true;
      try {
        status.textContent = "DOCX 내용을 읽는 중입니다...";
        var mammoth = await ensureMammoth(ctx);
        var result = await mammoth.convertToHtml({ arrayBuffer: await selectedFile.arrayBuffer() });

        if (!String(result.value || "").trim()) {
          throw new Error("DOCX에서 변환할 내용을 찾지 못했습니다.");
        }

        var pdf = await renderHtmlToPdfBlob(result.value, ctx, function (message) {
  ...
  async function openPdfSvg(x, ctx) {
    injectTextPdfStyle();
    ...
    ctx.stage.innerHTML =
      '<div class="hm-fx-detail">' +
        ...
        ctx.titleBlock(x) +
        '<div class="hm-pdf-box">' +
          '<div class="hm-pdf-drop" data-ps-drop><strong>PDF 파일을 선택하거나 끌어다 놓으세요</strong>' +
            '<span>선택한 한 페이지를 렌더링해 SVG 컨테이너로 저장합니다.</span>' +
            '<input type="file" hidden data-ps-file accept=".pdf,application/pdf"></div>' +
          '<div class="hm-pdf-fileinfo" data-ps-info></div>' +
          '<div class="hm-pdf-options">' +
            '<div class="hm-pdf-field"><label>페이지</label><input type="number" value="1" min="1" step="1" data-ps-page></div>' +
            '<div class="hm-pdf-field"><label>해상도</label><select data-ps-scale><option value="1.25">보통</option><option value="1.75" selected>선명</option><option value="2.5">고해상도</option></select></div>' +
          '</div>' +
          '<div class="hm-pdf-actions"><button class="hm-pdf-btn primary" type="button" data-ps-run disabled>SVG 만들기</button></div>' +
          '<div class="hm-pdf-status" data-ps-status>이 기능은 PDF 벡터 객체를 SVG 벡터로 재구성하는 것이 아니라 페이지를 이미지로 렌더링해 SVG 안에 담습니다.</div>' +
          '<div class="hm-pdf-help">PDF → SVG의 정확한 벡터 복원은 브라우저에서 문서별 차이가 커서 이번 기능은 래스터 기반 SVG로 명확히 구분했습니다.</div>' +
          '<div class="hm-pdf-version">PDF Engine v1.2.0</div>' +
        '</div></div>';

    var drop = ctx.stage.querySelector("[data-ps-drop]");
    var input = ctx.stage.querySelector("[data-ps-file]");
    var info = ctx.stage.querySelector("[data-ps-info]");
    var pageInput = ctx.stage.querySelector("[data-ps-page]");
    var scaleInput = ctx.stage.querySelector("[data-ps-scale]");
    var run = ctx.stage.querySelector("[data-ps-run]");
    var status = ctx.stage.querySelector("[data-ps-status]");
    ...
    async function choose(file) {
      try {
        ...
        info.textContent = file.name + " · " + niceBytes(file.size) + " · " + pageCount + "페이지";
        run.disabled = false;
        status.textContent = "변환할 페이지를 선택하세요.";
      } catch (error) {
        selectedFile = null;
        run.disabled = true;
        status.textContent = error.message || error;
    ...
    run.onclick = async function () {
      ...
      run.disabled = true;
      try {
        var pno = Math.max(1, Math.min(pageCount, Number(pageInput.value) || 1));
        var scale = Number(scaleInput.value) || 1.75;
        var pdfjs = await ensurePdfJs(ctx);
        ...
        status.textContent = pno + "페이지를 렌더링하는 중입니다...";
        ...
        var svg =
          ...
          '<image width="' + canvas.width + '" height="' + canvas.height + '" href="' + pngData + '" xlink:href="' + pngData + '"/>' +
          '</svg>';

        downloadBlob(
          new Blob([svg], { type:"image/svg+xml;charset=utf-8" }),
          safeBaseName(selectedFile.name) + "-page-" + pno + ".svg"
        );

        if (page.cleanup) page.cleanup();
        if (pdf.destroy) await pdf.destroy();

        status.textContent = pno + "페이지 SVG 다운로드를 시작했습니다.";
      } catch (error) {
        status.textContent = "SVG 생성 중 오류가 발생했습니다: " + (error.message || error);
      } finally {
        run.disabled = false;
      }
    };
  ...
  async function openSvgPdf(x, ctx) {
    injectTextPdfStyle();
    var selectedFile = null;
    ...
    ctx.stage.innerHTML =
      '<div class="hm-fx-detail">' +
        ...
        ctx.titleBlock(x) +
        '<div class="hm-pdf-box">' +
          '<div class="hm-pdf-drop" data-sp-drop><strong>SVG 파일을 선택하거나 끌어다 놓으세요</strong>' +
            '<span>SVG를 렌더링해 한 페이지 PDF로 만듭니다.</span>' +
            '<input type="file" hidden data-sp-file accept=".svg,image/svg+xml"></div>' +
          '<div class="hm-pdf-fileinfo" data-sp-info></div>' +
          '<div class="hm-pdf-options">' +
            '<div class="hm-pdf-field"><label>페이지 여백</label><select data-sp-margin><option value="0">없음</option><option value="10" selected>10mm</option><option value="20">20mm</option></select></div>' +
          '</div>' +
          '<div class="hm-pdf-actions"><button class="hm-pdf-btn primary" type="button" data-sp-run disabled>PDF 만들기</button></div>' +
          '<div class="hm-pdf-status" data-sp-status>SVG 내부의 외부 이미지·외부 폰트는 브라우저 보안 정책에 따라 표시되지 않을 수 있습니다.</div>' +
          '<div class="hm-pdf-help">PDF에는 SVG 렌더링 결과를 고해상도 이미지로 삽입합니다. 원본 SVG의 DOM이나 개별 벡터 객체를 PDF 편집 객체로 변환하는 기능은 아닙니다.</div>' +
          '<div class="hm-pdf-version">PDF Engine v1.2.0</div>' +
        '</div></div>';

    var drop = ctx.stage.querySelector("[data-sp-drop]");
    var input = ctx.stage.querySelector("[data-sp-file]");
    var info = ctx.stage.querySelector("[data-sp-info]");
    var marginInput = ctx.stage.querySelector("[data-sp-margin]");
    var run = ctx.stage.querySelector("[data-sp-run]");
    var status = ctx.stage.querySelector("[data-sp-status]");

    function choose(file) {
      if (!file || !/\.svg$/i.test(file.name)) {
        selectedFile = null;
        run.disabled = true;
        status.textContent = "SVG 파일을 선택해 주세요.";
        return;
      }
      if (file.size > 20 * 1024 * 1024) {
        selectedFile = null;
        run.disabled = true;
        status.textContent = "SVG 파일은 20MB 이하만 처리할 수 있습니다.";
        return;
      }
      selectedFile = file;
      info.style.display = "block";
    ...
    run.onclick = async function () {
      ...
      run.disabled = true;
      try {
        var loaded = await loadImage(selectedFile);
        try {
          var img = loaded.img;
          var wPx = img.naturalWidth || 1200;
          var hPx = img.naturalHeight || 1200;

          if (!wPx || !hPx) throw new Error("SVG 크기를 확인하지 못했습니다.");

          var scale = Math.min(2, 2400 / Math.max(wPx, hPx));
          ...
          var dims = fixedPage("a4", orientation);
          var margin = Number(marginInput.value) || 0;

          var drawScale = Math.min(
            (dims[0] - margin * 2) / canvas.width,
            (dims[1] - margin * 2) / canvas.height
          );
          var drawW = canvas.width * drawScale;
          var drawH = canvas.height * drawScale;
          var drawX = (dims[0] - drawW) / 2;
          var drawY = (dims[1] - drawH) / 2;

          var pdf = new JsPDF({ orientation:orientation, unit:"mm", format:"a4", compress:true });
  ...
  NS.pdf = {
    ...
    open: function (x, ctx) {
      ...
      if (x.engine === "pdf-docx-basic") return openPdfDocx(x, ctx);
      if (x.engine === "docx-pdf-basic") return openDocxPdf(x, ctx);
      if (x.engine === "pdf-svg-raster") return openPdfSvg(x, ctx);
      if (x.engine === "svg-pdf-raster") return openSvgPdf(x, ctx);
