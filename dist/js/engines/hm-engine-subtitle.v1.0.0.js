...
function cueCount(engine,out){
  try{
    ...
    if(engine==="vtt-srt"||engine==="ass-srt"||engine==="ssa-srt"||engine==="txt-srt")return parseSrt(out).length;
  ...
  open:function(x,ctx){
    ...
    stage.innerHTML='<div class="hm-fx-detail">'+
      ...
      '<div class="hm-fx-workbox">'+
        ...
        '<div class="hm-sub-options '+(x.engine==="txt-srt"?"is-show":"")+'" data-options>'+
          '<label>자막 한 줄 표시 시간<select data-duration><option value="2">2초</option><option value="3" selected>3초</option><option value="4">4초</option><option value="5">5초</option><option value="6">6초</option></select></label>'+
          '<label>생성 방식<select disabled><option>한 줄 = 자막 1개</option></select></label>'+
        '</div>'+
    ...
    var run=stage.querySelector("[data-run]"),copy=stage.querySelector("[data-copy]"),dl=stage.querySelector("[data-download]"),msg=stage.querySelector("[data-msg]"),meta=stage.querySelector("[data-meta]"),dur=stage.querySelector("[data-duration]");
    var objectUrl="";
    ...
    run.onclick=function(){
      ...
      try{
        var result=convert(x.engine,inp.value,{duration:dur&&dur.value});
        out.value=result;
